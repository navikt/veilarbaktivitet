...
public class AktivitetServiceTest {
	...
	public void setup() {
		aktivitetService =
			new AktivitetService(
				aktivitetDAO,
				new KvpService(kvpClient),
				metricService
			);
	}
	...
	public void opprettAktivitet() {
		...
		assertThat(
			getCapturedAktivitet().getFraDato(),
			equalTo(aktivitet.getFraDato())
		);
		assertThat(
			getCapturedAktivitet().getTittel(),
			equalTo(aktivitet.getTittel())
		);

		assertThat(getCapturedAktivitet().getKontorsperreEnhetId(), nullValue());
		assertThat(
			getCapturedAktivitet().getAktorId(),
			equalTo(KJENT_AKTOR_ID.get())
		);
		assertThat(
			getCapturedAktivitet().getTransaksjonsType(),
			equalTo(AktivitetTransaksjonsType.OPPRETTET)
		);
	...
	public void opprettAktivitetMedKvp() {
		...
		assertThat(
			getCapturedAktivitet().getKontorsperreEnhetId(),
			equalTo(kvp.getEnhet())
		);
	}
	...
	public void oppdaterStatus() {
		...
		captureInsertAktivitetArgument();
		assertThat(
			getCapturedAktivitet().getBeskrivelse(),
			equalTo(aktivitet.getBeskrivelse())
		);
		assertThat(getCapturedAktivitet().getStatus(), equalTo(nyStatus));
		assertThat(
			getCapturedAktivitet().getAvsluttetKommentar(),
			equalTo(avsluttKommentar)
		);
	}
	...
	public void oppdaterStatusMedKvpTilgang() {
		...
		val nyStatus = AktivitetStatus.GJENNOMFORES;
		val oppdatertAktivitet = kvpAktivitet.toBuilder().status(nyStatus).build();
	...
	public void oppdaterAktivitetFrist() {
		...
		aktivitetService.oppdaterAktivitetFrist(
			aktivitet,
			aktivitet.toBuilder().tilDato(nyFrist).build(),
			null
		);
	...
	public void oppdaterMoteTidOgSted() {
		...
		aktivitetService.oppdaterMoteTidStedOgKanal(
			...
			aktivitet
				.withTilDato(nyFrist)
				.withFraDato(nyFrist)
				.withMoteData(aktivitet.getMoteData().withAdresse(nyAdresse)),
			null
		);
		...
		assertThat(
			capturedAktivitet.getMoteData().getAdresse(),
			equalTo(nyAdresse)
		);
	}
	...
	public void oppdaterAktivitet() {
		...
		assertThat(
			getCapturedAktivitet().getBeskrivelse(),
			equalTo(oppdatertAktivitet.getBeskrivelse())
		);
		assertThat(
			getCapturedAktivitet().getLenke(),
			equalTo(oppdatertAktivitet.getLenke())
		);
	}
	...
	public void oppdaterAktivitet_skal_gi_versjonsKonflikt_hvis_to_oppdaterer_aktiviteten_samtidig() {
		val aktivitet = lagEnNyAktivitet();
		doThrow(new DuplicateKeyException("versjon fins"))
			.when(aktivitetDAO)
			.insertAktivitet(any());
	...
	public void oppdaterAktivitet_skal_sette_rett_transaksjonstype() {
		...
		assertThat(
			getCapturedAktivitet().getTransaksjonsType(),
			equalTo(AktivitetTransaksjonsType.DETALJER_ENDRET)
		);

		aktivitetService.oppdaterAktivitet(
			aktivitet,
			aktivitet.toBuilder().avtalt(true).build(),
			null
		);
		captureInsertAktivitetArgument();
		assertThat(
			getCapturedAktivitet().getTransaksjonsType(),
			equalTo(AktivitetTransaksjonsType.AVTALT)
		);
	}
	...
	public void settAktiviteterTilHistoriske_ingenHistoriskDato_oppdaterAktivitet() {
		...
		aktivitetService.settAktiviteterTilHistoriske(
	...
	public void settAktiviteterTilHistoriske_opprettetEtterSluttDato_ikkeOppdaterAktivitet() {
		...
		gitt_aktivitet(
			lagEnNyAktivitet().withOpprettetDato(new Date(sluttdato.getTime() + 1))
		);
		aktivitetService.settAktiviteterTilHistoriske(
			Person.aktorId("aktorId"),
			sluttdato
	...
	private void gitt_aktivitet(AktivitetData aktivitetData) {
		when(aktivitetDAO.hentAktiviteterForAktorId(any(Person.AktorId.class)))
			.thenReturn(asList(aktivitetData));
	}
	...
	public void captureInsertAktivitetArgument() {
		Mockito
			.verify(aktivitetDAO, atLeastOnce())
			.insertAktivitet((AktivitetData) argumentCaptor.capture());
	}
