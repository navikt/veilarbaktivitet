...
public class ArenaAktivitetConsumerTest {
	...
	public void springInitialiseringSkalLeseVariabelOgParseDato() {
		clearProperty(
			no.nav.veilarbaktivitet.config.ApplicationContext.ARENA_AKTIVITET_DATOFILTER_PROPERTY
		);
		ApplicationContext context = setupContext(mock(TiltakOgAktivitetV1.class));
		ArenaAktivitetConsumer consumer = context.getBean(
		...
		assertThat(
			consumer.arenaAktivitetFilterDato,
			equalTo(ArenaAktivitetConsumer.parseDato("2017-08-30"))
		);
	}
	...
	public void skalFiltrereArenaAktiviteterBasertPaaDato() throws Exception {
		...
		HentTiltakOgAktiviteterForBrukerResponse responsMedNyAktivitet = responsMedTiltak(
			new Date(arenaAktivitetFilterDato.getTime() + 1)
		);
		when(
				arena.hentTiltakOgAktiviteterForBruker(
					any(HentTiltakOgAktiviteterForBrukerRequest.class)
				)
			)
			.thenReturn(responsMedNyAktivitet);
		assertThat(
			consumer.hentArenaAktiviteter(Person.fnr("123")).size(),
			equalTo(1)
		);

		HentTiltakOgAktiviteterForBrukerResponse responsMedGammelAktivitet = responsMedTiltak(
			new Date(arenaAktivitetFilterDato.getTime() - 1)
		);
		when(
				arena.hentTiltakOgAktiviteterForBruker(
	...
	@Test
	public void skalIkkeFiltrereArenaAktiviteterHvisFilterDatoErNull()
		throws Exception {
		TiltakOgAktivitetV1 arena = mock(TiltakOgAktivitetV1.class);
		ArenaAktivitetConsumer consumer = new ArenaAktivitetConsumer(arena);
	...
	private HentTiltakOgAktiviteterForBrukerResponse responsMedTiltak(Date date) {
		HentTiltakOgAktiviteterForBrukerResponse arenaResponse = mock(
			HentTiltakOgAktiviteterForBrukerResponse.class
		);
		...
		when(arenaResponse.getTiltaksaktivitetListe())
			.thenReturn(Arrays.asList(tiltak));
	...
	public void skalViseTiltaksvariantPaaGruppeAMO() throws Exception {
		...
		responsMedNyAktivitet
			.getTiltaksaktivitetListe()
			.get(0)
			.setTiltaksnavn("Gruppe AMO");
		...
		assertThat(
			aktivitet.getTittel(),
			equalTo("Gruppe AMO: " + aktivitet.getTiltakLokaltNavn())
		);
	}
}
