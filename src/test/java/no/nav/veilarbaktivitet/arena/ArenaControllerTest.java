...
public class ArenaControllerTest {
	...
	private final ArenaAktivitetConsumer consumer = mock(
		ArenaAktivitetConsumer.class
	);

	private final JdbcTemplate jdbc = LocalH2Database.getDb();
	private final ArenaForhaandsorienteringDAO dao = new ArenaForhaandsorienteringDAO(
		new Database(jdbc)
	);
	private final ArenaService service = new ArenaService(
		consumer,
		dao,
		authService
	);
	private final ArenaController controller = new ArenaController(
		context,
		authService,
		service
	);
	...
	private final Forhaandsorientering forhaandsorientering = Forhaandsorientering
		.builder()
		.type(Forhaandsorientering.Type.SEND_FORHAANDSORIENTERING)
		.tekst("kake")
		.build();
	...
	public void cleanup() {
		...
		doNothing().when(authService).sjekkTilgangTilPerson(fnr);

		when(authService.erInternBruker()).thenReturn(true);
		when(authService.getAktorIdForPersonBrukerService(fnr))
			.thenReturn(Optional.of(aktorid));
		when(authService.getAktorIdForPersonBrukerService(ikkeTilgangFnr))
			.thenReturn(Optional.of(ikkeTilgangAktorid));
	...
	public void sendForhaandsorienteringSkalFeileUtenForhaandsorientering() {
		ResponseStatusException exception = assertThrows(
			...
			() -> controller.sendForhaandsorientering(null, "arenaAktivitetId")
		);
		assertEquals(
			"forhaandsorientering kan ikke være null",
			exception.getReason()
		);
	...
	public void sendForhaandsorienteringSkalFeileUtenForhaandsorienteringsType() {
		ResponseStatusException exception = assertThrows(
			ResponseStatusException.class,
			() ->
				controller.sendForhaandsorientering(
					Forhaandsorientering.builder().build(),
					"arenaAktivitetId"
				)
		);
		assertEquals(
	...
	public void sendForhaandsorienteringSkalFeileUtenArenaAktivitet() {
		ResponseStatusException exception = assertThrows(
			ResponseStatusException.class,
			() -> controller.sendForhaandsorientering(null, null)
		);
		assertEquals(
			"arenaaktivitetId kan ikke være null eller tom",
			exception.getReason()
		);
		assertEquals(HttpStatus.BAD_REQUEST, exception.getStatus());
	}
	...
	public void sendForhaandsorienteringSkalFeileHvisArenaAktivitetenIkkeFinnes() {
		...
		ResponseStatusException exception = assertThrows(
			ResponseStatusException.class,
			() ->
				controller.sendForhaandsorientering(forhaandsorientering, "Arena123")
		);
	...
	public void sendForhaandsorienteringSkalreturnereAktivitetMedForhaandsorientering() {
		...
		ArenaAktivitetDTO arenaAktivitetDTO = controller.sendForhaandsorientering(
			forhaandsorientering,
			tilFho.getId()
		);

		assertEquals(
			tilFho.setForhaandsorientering(forhaandsorientering),
			arenaAktivitetDTO
		);
	}
	...
	public void sendForhaandsorienteringSkalOppdaterehentArenaAktiviteter() {
		...
		List<ArenaAktivitetDTO> forventet = List.of(
			tilFho.setForhaandsorientering(forhaandsorientering),
			ikkeTilFho
		);
	...
	public void hentArenaAktiviteterSkalReturnereArenaAktiviteter() {
		...
		when(consumer.hentArenaAktiviteter(fnr)).thenReturn(tiltak);
	...
	public void markerForhaandsorienteringSomLestSkalOppdatereArenaAktivitet() {
		...
		when(consumer.hentArenaAktiviteter(fnr)).thenReturn(List.of(a1));

		ArenaAktivitetDTO sendtAktivitet = controller.sendForhaandsorientering(
			forhaandsorientering,
			a1.getId()
		);
	...
	public void tilgangskontrollPaaHentArenaAktiviteterSkalFinnes() {
		...
		when(consumer.hentArenaAktiviteter(ikkeTilgangFnr)).thenReturn(tiltak);

		when(context.getFnr()).thenReturn(Optional.of(ikkeTilgangFnr));

		ResponseStatusException exception = assertThrows(
			ResponseStatusException.class,
			controller::hentArenaAktiviteter
		);
