...
public class KafkaAktivitetDAOTest {
	...
	private final KafkaAktivitetDAO kafkaAktivitetDAO = new KafkaAktivitetDAO(
		database
	);
	...
	public void hentOppTil5000MeldingerSomIkkeErSendt_AktivitetErSendOgIkkeSendt_returnererKunUsendte() {
		...
		Assert.assertTrue(
			containsVersion(usendteAktiviteter_pre, usendt.getVersjon())
		);
		Assert.assertTrue(
			containsVersion(usendteAktiviteter_pre, sendt.getVersjon())
		);
		...
		Assert.assertTrue(
			containsVersion(usendteAktiviteter_post, usendt.getVersjon())
		);
		Assert.assertFalse(
			containsVersion(usendteAktiviteter_post, sendt.getVersjon())
		);
	}
	...
	public void hentOppTil5000MeldingerSomIkkeErSendt_AktivitetErIkkeSendt_returnererEnAktivitet() {
		...
		Assert.assertTrue(
			containsVersion(usendteAktiviteter, aktivitet.getVersjon())
		);
	}
	...
	public void hentOppTil5000MeldingerSomIkkeErSendt_kunSendteAktiviteter_returnererIngen() {
		...
		Assert.assertFalse(
			containsVersion(usendteAktiviteter_tom, aktivitet.getVersjon())
		);
	}

	private boolean containsVersion(
		List<KafkaAktivitetMeldingV4> list,
		Long version
	) {
		return (
			list.stream().filter(e -> e.getVersion().equals(version)).count() == 1
		);
	}
	...
	private AktivitetData insertAktivitet(AktivitetData aktivitet) {
		val id = Optional
			.ofNullable(aktivitet.getId())
			.orElseGet(aktivitetDAO::getNextUniqueAktivitetId);
		val aktivitetMedId = aktivitet
			.toBuilder()
			.id(id)
