...
public class AktivitetDataTestBuilder {
	...
	public static AktivitetData nyttStillingssøk() {
		return AktivitetDataTestBuilder
			.nyAktivitet()
			.aktivitetType(AktivitetTypeData.JOBBSOEKING)
			.stillingsSoekAktivitetData(
				AktivitetTypeDataTesBuilder.nyttStillingssøk()
			)
			.build();
	}
	...
	public static AktivitetData nySokeAvtaleAktivitet() {
		return AktivitetDataTestBuilder
			...
			.sokeAvtaleAktivitetData(
				AktivitetTypeDataTesBuilder.nySokeAvtaleAktivitet()
			)
			.build();
	}
	...
	public static AktivitetData nyBehandlingAktivitet() {
		return AktivitetDataTestBuilder
			...
			.behandlingAktivitetData(
				AktivitetTypeDataTesBuilder.nyBehandlingAktivitet()
			)
			.build();
	}
	...
	public static Date nyDato() {
		return truncate(
			new Date(new Random().nextLong() % System.currentTimeMillis()),
			SECOND
		);
	}
}
