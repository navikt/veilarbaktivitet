...
public class InsertAktiviteter {
	...
	public static void insertEnAvHver(String akotrid, AktivitetDAO aktivitetDAO) {
		insertNyAktivitet(
			AktivitetDataTestBuilder.nyBehandlingAktivitet(),
			akotrid,
			aktivitetDAO
		);
		insertNyAktivitet(
			AktivitetDataTestBuilder.nyIJobbAktivitet(),
			akotrid,
			aktivitetDAO
		);
		insertNyAktivitet(
			AktivitetDataTestBuilder.nyMoteAktivitet(),
			akotrid,
			aktivitetDAO
		);
		insertNyAktivitet(
			AktivitetDataTestBuilder.nySokeAvtaleAktivitet(),
			akotrid,
			aktivitetDAO
		);
		insertNyAktivitet(
		...
		insertNyAktivitet(
			AktivitetDataTestBuilder.nytSamtaleReferat(),
			akotrid,
			aktivitetDAO
		);
		insertNyAktivitet(
			AktivitetDataTestBuilder.nyttStillingssøk(),
			akotrid,
			aktivitetDAO
		);
	}

	private static void insertNyAktivitet(
		AktivitetData aktivitetData,
		String aktorId,
		AktivitetDAO aktivitetDAO
	) {
		long id = aktivitetDAO.getNextUniqueAktivitetId();
		AktivitetData build = aktivitetData
			.toBuilder()
			.aktorId(aktorId)
			.id(id)
			.build();
