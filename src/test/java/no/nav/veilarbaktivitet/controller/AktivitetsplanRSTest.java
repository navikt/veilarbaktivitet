...
public class AktivitetsplanRSTest {
	...
	private AktivitetService aktivitetService = new AktivitetService(
		aktivitetDAO,
		kvpService,
		metricService
	);
	...
	private AktivitetAppService appService = new AktivitetAppService(
		arenaService,
		authService,
		aktivitetService,
		metricService
	...
	private AktivitetsplanController aktivitetController = new AktivitetsplanController(
		authService,
		appService,
	...
	public AuthContextRule authContextRule = new AuthContextRule(
		AuthTestUtils.createAuthContext(UserRole.INTERN, "testident")
	);

	@Before
	public void setup() {
		when(authService.getAktorIdForPersonBrukerService(any()))
			.thenReturn(Optional.of(KJENT_AKTOR_ID));
	...
	public void hentHarTiltak_harAktiveTiltak_returnererTrue() {
		...
		when(tiltakOgAktivitet.hentTiltakOgAktiviteterForBruker(any()))
			.thenReturn(responseMock);

		ArenaAktivitetConsumer arenaAktivitetConsumerAktiv = new ArenaAktivitetConsumer(
			tiltakOgAktivitet
		);
		ArenaService arenaService = new ArenaService(
			arenaAktivitetConsumerAktiv,
			new ArenaForhaandsorienteringDAO(database),
			authService
		);
		AktivitetAppService aktivitetAppService = new AktivitetAppService(
			arenaService,
			authService,
			aktivitetService,
			metricService
		);
		AktivitetsplanController aktivitetsplanController = new AktivitetsplanController(
			authService,
			aktivitetAppService,
			mockHttpServletRequest
		);
	...
	public void skal_ikke_kunne_endre_annet_enn_frist_pa_avtalte_aktiviter() {
		...
		when(authService.getInnloggetBrukerIdent())
			.thenReturn(Optional.of(KJENT_IDENT.get()));

		AuthContextHolderThreadLocal
			.instance()
			.withContext(
				AuthTestUtils.createAuthContext(UserRole.INTERN, KJENT_IDENT.get()),
				() -> {
					gitt_at_jeg_har_laget_en_aktivtet();
	...
	private void gitt_at_jeg_har_folgende_aktiviteter(
		List<AktivitetData> aktiviteter
	) {
		lagredeAktivitetsIder =
			aktiviteter
				.stream()
				.map(
					aktivitet ->
	...
	private void nar_jeg_oppdaterer_aktiviten() {
		...
		aktivitet =
			aktivitetController.oppdaterAktivitet(
				aktivitet
					.setBeskrivelse("noe tull")
	...
	private void nar_jeg_flytter_en_aktivitet_til_en_annen_status() {
		...
		this.aktivitet =
			aktivitetController.oppdaterStatus(
				aktivitet.setStatus(nyAktivitetStatus)
			);
	}
	...
	private void nar_jeg_oppdaterer_etiketten_pa_en_aktivitet() {
		val aktivitet = aktivitetController.hentAktivitetsplan().aktiviteter.get(0);
		this.aktivitet =
			aktivitetController.oppdaterEtikett(
				aktivitet.setEtikett(nyAktivitetEtikett)
			);
	}
	...
	private void nar_jeg_oppdaterer_en_av_aktiviten() {
		val originalAktivitet = aktivitetService.hentAktivitet(
			lagredeAktivitetsIder.get(0)
		);
		oldOpprettetDato = originalAktivitet.getOpprettetDato();
		nyLenke = "itsOver9000.com";
		nyAvsluttetKommentar =
		...
		this.aktivitet =
			aktivitetController.oppdaterAktivitet(
				AktivitetDTOMapper.mapTilAktivitetDTO(nyAktivitet)
			);
	...
	private void da_skal_disse_aktivitene_ligge_i_min_aktivitetsplan() {
		List<AktivitetDTO> aktiviteter = aktivitetController.hentAktivitetsplan()
			.aktiviteter;
	...
	private void da_skal_jeg_kunne_hente_en_aktivitet() {
		assertThat(
			lagredeAktivitetsIder.get(0).toString(),
			equalTo(
				(
					(AktivitetDTO) aktivitetController.hentAktivitet(
						lagredeAktivitetsIder.get(0).toString()
					)
				).getId()
			)
	...
	private void da_skal_jeg_denne_aktivteten_ligge_i_min_aktivitetsplan() {
		assertThat(
			aktivitetService.hentAktiviteterForAktorId(KJENT_AKTOR_ID),
			hasSize(1)
		);
	}
	...
	private void da_skal_min_aktivitet_fatt_ny_status() {
		...
		assertThat(
			aktivitetService
				.hentAktivitet(Long.parseLong(aktivitet.getId()))
				.getStatus(),
	...
	private void da_skal_jeg_aktiviten_vare_endret() {
		val lagretAktivitet = (AktivitetDTO) aktivitetController.hentAktivitet(
			this.lagredeAktivitetsIder.get(0).toString()
		);
		assertThat(lagretAktivitet.getLenke(), equalTo(nyLenke));
		assertThat(
			lagretAktivitet.getAvsluttetKommentar(),
			equalTo(nyAvsluttetKommentar)
		);
		...
		assertThat(
			...
			equalTo(
				orignalAktivitet
					...
					.setEndretAv(
						AuthContextHolderThreadLocal.instance().getSubject().orElseThrow()
					)
			)
		);
	}
