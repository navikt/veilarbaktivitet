...
public class AvsluttetOppfolgingFeedProviderTest {
	...
	private AvsluttetOppfolgingFeedDAO avsluttetOppfolgingFeedDAO = mock(
		AvsluttetOppfolgingFeedDAO.class
	);
	private AvsluttetOppfolgingFeedConsumer feedProvider = new AvsluttetOppfolgingFeedConsumer(
		aktivitetService,
		avsluttetOppfolgingFeedDAO
	);
	...
	public void skal_ta_vare_paa_siste_kjente_id() {
		...
		feedProvider.lesAvsluttetOppfolgingFeed(
			null,
			asList(element(oppdatert1), element(oppdatert2))
		);
	...
	public void skal_ikke_oppdatere_id_hvis_exception() {
		doThrow(new RuntimeException())
			.when(aktivitetService)
			.settAktiviteterTilHistoriske(AKTØR_ID, null);

		try {
			feedProvider.lesAvsluttetOppfolgingFeed(
				null,
				asList(element(new Date()))
			);
		} finally {
