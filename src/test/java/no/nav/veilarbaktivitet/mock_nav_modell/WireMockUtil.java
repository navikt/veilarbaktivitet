...
public class WireMockUtil {
    ...
    private static void oppfolging(String fnr, Person.AktorId aktorId, boolean underOppfolging, boolean oppfolgingFeiler, Oppfolgingsperiode nyesteOppfølgingsperiode) {
        if (oppfolgingFeiler) {
            ...
            wireMock.stubFor(get("/veilarboppfolging/api/v2/oppfolging/periode/gjeldende?fnr="
                    + fnr)
                    .willReturn(aResponse().withStatus(500)));
            wireMock.stubFor(get("/veilarboppfolging/api/v2/oppfolging/perioder?aktorId=" + aktorId.get())
                    .willReturn(aResponse().withStatus(500)));
            return;
        }
        wireMock.stubFor(get("/veilarboppfolging/api/v2/oppfolging?fnr=" + fnr)
    ...
    private static void forhaandsvisningSendTilBruker() {
        wireMock.stubFor(post("/orkivar/forhaandsvisning-send-til-bruker")
                .willReturn(aResponse()
                        .withBody("""
                                {
                                   "pdf":"%s",
    ...
    private static void hentSak(Long sakId, Oppfolgingsperiode oppfolgingsperiode) {
        if (oppfolgingsperiode != null) {
            ...
            wireMock.stubFor(post(urlMatching("/veilarboppfolging/api/v3/sak/" + oppfolgingsperiodeId))
                    .willReturn(ok()
                            .withHeader("Content-Type", "text/json")
                            .withBody("""
                                    {
