...
class AktivitetDtoArkivPayloadTest {
    ...
    fun `Stilling fra NAV har riktige felt`() {
        val mote = AktivitetDataTestBuilder.nyStillingFraNav().toArkivPayload(null, Historikk(emptyList()))
        assertThat(mote.detaljer.map { it.tittel })
            .contains(
                "Arbeidsgiver",
                "Arbeidssted",
    ...
    fun `Jobbrettet egenaktivitet har riktige felt`() {
        val mote = AktivitetDataTestBuilder.nyEgenaktivitet().toArkivPayload(null, Historikk(emptyList()))
        assertThat(mote.detaljer.map { it.tittel })
            .containsExactly(
                "Fra dato",
                "Til dato",
    ...
    fun `Jobbsøking (sokeavtale) egenaktivitet har riktige felt`() {
        val mote = AktivitetDataTestBuilder.nySokeAvtaleAktivitet().toArkivPayload(null, Historikk(emptyList()))
        assertThat(mote.detaljer.map { it.tittel })
            .containsExactly(
                "Fra dato",
                "Til dato",
    ...
    fun `Behandling skal ha riktige felt`() {
        val behandling = AktivitetDataTestBuilder.nyBehandlingAktivitet().toArkivPayload(null, Historikk(emptyList()))
    ...
    fun `IJobb (jobb jeg har) skal ha riktige felt`() {
        val ijobb = AktivitetDataTestBuilder.nyIJobbAktivitet().toArkivPayload(null, Historikk(emptyList()))
