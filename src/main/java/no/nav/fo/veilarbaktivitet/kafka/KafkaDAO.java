...
public class KafkaDAO {
    ...
    public void lagre(KafkaAktivitetMelding melding) {
        SqlUtils.insert(jdbcTemplate, TABLE_NAME)
                ...
                .value("FRA_DATO", Timestamp.from(melding.getFraDato()))
                .value("TIL_DATO", Timestamp.from(melding.getFraDato()))
                .value("ENDRET_DATO", Timestamp.from(melding.getFraDato()))
    ...
    private SQLFunction<ResultSet, KafkaAktivitetMelding> mapper() {
        return rs -> KafkaAktivitetMelding.builder()
                ...
                .fraDato(rs.getTimestamp("FRA_DATO").toInstant())
                .tilDato(rs.getTimestamp("TIL_DATO").toInstant())
                .endretDato(rs.getTimestamp("ENDRET_DATO").toInstant())
