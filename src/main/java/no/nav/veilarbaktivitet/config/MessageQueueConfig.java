...
public class MessageQueueConfig {
	private static final String MQGATEWAY03_HOSTNAME_PROPERTY =
	...
	private static final String VARSELPRODUKSJON_VARSLINGER_QUEUENAME_PROPERTY =
	...
	public JmsTemplate varselQueue(ConnectionFactory connectionFactory) {
		return queue(
			connectionFactory,
			getRequiredProperty(VARSELPRODUKSJON_VARSLINGER_QUEUENAME_PROPERTY)
		);
	}
	...
	public ConnectionFactory connectionFactory() throws JMSException {
		...
		String env = requireNamespace().equals("default")
			? "p"
			: requireNamespace();

		connectionFactory.setStringProperty(
			WMQConstants.WMQ_HOST_NAME,
			getRequiredProperty(MQGATEWAY03_HOSTNAME_PROPERTY)
		);
		connectionFactory.setStringProperty(
			WMQConstants.WMQ_PORT,
			getRequiredProperty(MQGATEWAY03_PORT_PROPERTY)
		);
		connectionFactory.setStringProperty(
			WMQConstants.WMQ_CHANNEL,
			String.format("%s_%s", env, requireApplicationName()).toUpperCase()
		);
		connectionFactory.setIntProperty(
			WMQConstants.WMQ_CONNECTION_MODE,
			WMQConstants.WMQ_CM_CLIENT
		);
		connectionFactory.setStringProperty(
			WMQConstants.WMQ_QUEUE_MANAGER,
			getRequiredProperty(MQGATEWAY03_NAME_PROPERTY)
		);
