...
public class KasserController {
	...
	private final String godkjenteIdenter = getOptionalProperty(
			VEILARB_KASSERING_IDENTER_PROPERTY
		)
		.orElse("");

	public KasserController(
	...
	public void kasserAktivitet(@PathVariable("aktivitetId") String aktivitetId) {
		...
		kjorHvisTilgang(
			...
			aktivitetId,
			() -> aktivitetDAO.kasserAktivitet(id)
		);
	}

	private boolean kjorHvisTilgang(
		String aktorId,
		String id,
		Supplier<Boolean> fn
	) {
		...
		if (!godkjente.contains(veilederIdent)) {
			...
			throw new ResponseStatusException(
				...
				String.format(
					"[KASSERING] %s har ikke tilgang til kassinger av %s aktivitet",
					veilederIdent,
					aktorId
				)
			);
