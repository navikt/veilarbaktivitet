...
public class AktivitetsplanController {
	...
	public AktivitetDTO hentAktivitet(@PathVariable("id") String aktivitetId) {
		return Optional
			.of(appService.hentAktivitet(Long.parseLong(aktivitetId)))
			.map(AktivitetDTOMapper::mapTilAktivitetDTO)
	...
	public List<AktivitetDTO> hentAktivitetVersjoner(
		@PathVariable("id") String aktivitetId
	) {
		return Optional
			.of(aktivitetId)
			.map(Long::parseLong)
	...
	@PostMapping("/ny")
	public AktivitetDTO opprettNyAktivitet(
		@RequestBody AktivitetDTO aktivitet,
		...
		return Optional
			...
			.map(
				aktivitetData ->
					appService.opprettNyAktivitet(getContextUserIdent(), aktivitetData)
			)
			.map(AktivitetDTOMapper::mapTilAktivitetDTO)
			.orElseThrow(RuntimeException::new);
	}
	...
	private Person getContextUserIdent() {
		if (authService.erEksternBruker()) {
			return authService
				.getInnloggetBrukerIdent()
				.map(Person::fnr)
				.orElseThrow(
					() ->
						new ResponseStatusException(
							HttpStatus.FORBIDDEN,
							"Fant ikke ident for innlogget bruker"
						)
				);
		}
