...
public class MetricService {
	...
	public void opprettNyAktivitetMetrikk(AktivitetData aktivitetData) {
		...
		Event ev = new Event("aktivitet.ny")
			...
			.addFieldToReport(
				"lagtInnAvNAV",
				aktivitetData.getLagtInnAv().equals(InnsenderData.NAV)
			)
			.addFieldToReport(
				"automatiskOpprettet",
				aktivitetData.isAutomatiskOpprettet()
			)
	...
	public void oppdaterAktivitetMetrikk(
		AktivitetData aktivitetData,
		boolean blittAvtalt,
		boolean erAutomatiskOpprettet
	) {
		String malId = Optional.ofNullable(aktivitetData.getMalid()).orElse("");
	...
	public void reportFilterAktivitet(
		AktivitetData aktivitetData,
		boolean harTilgang
	) {
		Event ev = new Event("aktivitet.filter")
			.addTagToReport(
				"kontorsperre",
				String.valueOf(aktivitetData.getKontorsperreEnhetId() != null)
			)
	...
	public void reportAktivitetLestAvBrukerForsteGang(
		AktivitetData aktivitetData
	) {
		String malId = Optional.ofNullable(aktivitetData.getMalid()).orElse("");
		...
		Event ev = new Event("aktivitet.lestAvBrukerForsteGang")
			...
			.addFieldToReport(
				"lestTidspunkt",
				aktivitetData.getLestAvBrukerForsteGang().getTime()
			)
			.addFieldToReport(
				"tidSidenOpprettet",
				tidMellomOpprettetOgLestForsteGang(aktivitetData)
			)
			.addFieldToReport("malId", malId);
		metricsClient.report(ev);
	}

	private void oppdatertStatus(
		AktivitetData aktivitetData,
		boolean oppdatertAvNAV
	) {
		String malId = Optional.ofNullable(aktivitetData.getMalid()).orElse("");
		Event ev = new Event("aktivitet.oppdatert.status")
			...
			.addFieldToReport(
				"tidSidenOpprettet",
				tidMellomOpprettetOgOppdatert(aktivitetData)
			)
			.addFieldToReport(
				"automatiskOpprettet",
				aktivitetData.isAutomatiskOpprettet()
	...
	private static long tidMellomOpprettetOgOppdatert(
		AktivitetData aktivitetData
	) {
		return (
			aktivitetData.getEndretDato().getTime() -
			aktivitetData.getOpprettetDato().getTime()
		);
	}

	private static long tidMellomOpprettetOgLestForsteGang(
		AktivitetData aktivitetData
	) {
		return (
			aktivitetData.getLestAvBrukerForsteGang().getTime() -
