...
public class AktivitetDAO {
	...
	public List<AktivitetData> hentAktiviteterForAktorId(Person.AktorId aktorId) {
		return database.query(
			SELECT_AKTIVITET + "WHERE A.aktor_id = ? and A.gjeldende = 1",
	...
	public AktivitetData hentAktivitet(long aktivitetId) {
		return database.queryForObject(
			SELECT_AKTIVITET + "WHERE A.aktivitet_id = ? and gjeldende = 1",
			AktivitetDataRowMapper::mapAktivitet,
			aktivitetId
		);
	}
	...
	public void insertAktivitet(AktivitetData aktivitet, Date endretDato) {
		...
		database.update(
			"UPDATE AKTIVITET SET gjeldende = 0 where aktivitet_id = ?",
			aktivitetId
		);
		...
		insertSokeAvtale(
			aktivitetId,
			versjon,
			aktivitet.getSokeAvtaleAktivitetData()
		);
	...
	private void insertStillingsSoek(
		long aktivitetId,
		long versjon,
		StillingsoekAktivitetData stillingsSoekAktivitet
	) {
	...
	private void insertEgenAktivitet(
		long aktivitetId,
		long versjon,
		...
		ofNullable(egenAktivitetData)
			.ifPresent(
				egen ->
					database.update(
						"INSERT INTO EGENAKTIVITET(aktivitet_id, versjon, hensikt, oppfolging) " +
						"VALUES(?,?,?,?)",
						aktivitetId,
						versjon,
						egen.getHensikt(),
	...
	private void insertIJobb(
		long aktivitetId,
		long versjon,
		IJobbAktivitetData iJobbAktivitet
	) {
	...
	private void insertBehandling(
		long aktivitetId,
		long versjon,
		BehandlingAktivitetData behandlingAktivitet
	) {
