...
public class AktivitetDataRowMapper {
		...
		val aktivitet = AktivitetData
			...
			.status(
				EnumUtils.valueOf(
					...
					rs.getString("livslopstatus_kode")
				)
			)
			...
			.lagtInnAv(
				EnumUtils.valueOf(InnsenderData.class, rs.getString("lagt_inn_av"))
			)
			...
			.forhaandsorientering(
				Forhaandsorientering
					...
					.type(
						EnumUtils.valueOf(
							Forhaandsorientering.Type.class,
							rs.getString("fho_type")
						)
					)
			...
			.transaksjonsType(
				EnumUtils.valueOf(
					AktivitetTransaksjonsType.class,
			...
			.lestAvBrukerForsteGang(
				Database.hentDato(rs, "lest_av_bruker_forste_gang")
			)
	...
	private static MoteData mapMoteData(ResultSet rs) throws SQLException {
		return MoteData
			...
			.kanal(
				Optional
					.ofNullable(rs.getString("kanal"))
					.map(KanalDTO::valueOf)
					.orElse(null)
			)
	...
	private static StillingsoekAktivitetData mapStillingsAktivitet(ResultSet rs)
		throws SQLException {
		return StillingsoekAktivitetData
			.builder()
			...
			.stillingsoekEtikett(
				EnumUtils.valueOf(
					StillingsoekEtikettData.class,
					rs.getString("etikett")
				)
			)
			.build();
	}

	private static EgenAktivitetData mapEgenAktivitet(ResultSet rs)
		throws SQLException {
		return EgenAktivitetData
			.builder()
	...
	private static SokeAvtaleAktivitetData mapSokeAvtaleAktivitet(ResultSet rs)
		throws SQLException {
		return SokeAvtaleAktivitetData
			.builder()
	...
	private static IJobbAktivitetData mapIJobbAktivitet(ResultSet rs)
		throws SQLException {
		return IJobbAktivitetData
			.builder()
			.jobbStatusType(
				EnumUtils.valueOf(JobbStatusTypeData.class, rs.getString("jobb_status"))
			)
	...
	private static BehandlingAktivitetData mapBehandlingAktivitet(ResultSet rs)
		throws SQLException {
		return BehandlingAktivitetData
			.builder()
