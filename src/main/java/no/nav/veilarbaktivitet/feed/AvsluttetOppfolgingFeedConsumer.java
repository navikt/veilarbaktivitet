...
public class AvsluttetOppfolgingFeedConsumer {
	...
	public AvsluttetOppfolgingFeedConsumer(
		AktivitetService aktivitetService,
		AvsluttetOppfolgingFeedDAO avsluttetOppfolgingFeedDAO
	) {
	...
	public String sisteKjenteId() {
		Date sisteKjenteId = ofNullable(
				avsluttetOppfolgingFeedDAO.hentSisteKjenteId()
			)
			.orElseGet(() -> new Date(0));
		return ZonedDateTime
			.ofInstant(sisteKjenteId.toInstant(), ZoneId.systemDefault())
			.toString();
	}

	public void lesAvsluttetOppfolgingFeed(
		String lastEntryId,
		List<AvsluttetOppfolgingFeedDTO> elements
	) {
			...
			.map(
				element -> {
					aktivitetService.settAktiviteterTilHistoriske(
						Person.aktorId(element.getAktoerid()),
						element.getSluttdato()
					);
