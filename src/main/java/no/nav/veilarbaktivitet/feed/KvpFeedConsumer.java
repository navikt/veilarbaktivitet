...
public class KvpFeedConsumer {
	...
	public KvpFeedConsumer(
		AktivitetService aktivitetService,
		KVPFeedDAO kvpFeedDAO
	) {
	...
	public void lesKvpFeed(String lastEntryId, List<KvpDTO> elements) {
		elements
			...
			.map(
				element -> {
					if (element.getAvsluttetDato() != null) {
						aktivitetService.settAktiviteterInomKVPPeriodeTilAvbrutt(
							Person.aktorId(element.getAktorId()),
							element.getAvsluttetBegrunnelse(),
							element.getAvsluttetDato()
						);
					}
			...
			.reduce((a1, a2) -> a2)
			.ifPresent(
				lastSuccessfulId -> kvpFeedDAO.oppdaterSisteKVPFeedId(lastSuccessfulId)
			);
	}
}
