...
public class FeedConsumer<DOMAINOBJECT extends Comparable<DOMAINOBJECT>>
	implements Authorization, ApplicationListener<ContextClosedEvent> {
	...
	public FeedConsumer(FeedConsumerConfig<DOMAINOBJECT> config) {
		...
		createScheduledJob(
			...
			runWithLock(feedName, this::poll)
		);
		createScheduledJob(
			feedName + "/webhook",
			host,
			config.webhookPollingConfig,
	...
	void registerWebhook() {
		String callbackUrl = callbackUrl(
			this.config.webhookPollingConfig.apiRootPath,
			this.config.feedName
		);
		FeedWebhookRequest body = new FeedWebhookRequest()
		.setCallbackUrl(callbackUrl);

		Entity<FeedWebhookRequest> entity = Entity.entity(
			body,
		...
		try (Response response = this.config.client.newCall(request).execute()) {
			...
			if (responseStatus == 201) {
				...
				LOG.warn(
					"Endepunkt for opprettelse av webhook returnerte feilkode {}",
					responseStatus
				);
			}
		}
	...
	public synchronized Response poll() {
		if (config.leaderElectionClient.isLeader()) {
			...
			HttpUrl.Builder httpBuilder = Objects
				.requireNonNull(HttpUrl.parse(getTargetUrl()))
				.newBuilder();
			httpBuilder.addQueryParameter(QUERY_PARAM_ID, lastEntry);
			httpBuilder.addQueryParameter(
				QUERY_PARAM_PAGE_SIZE,
				String.valueOf(this.config.pageSize)
			);

			Request request = new Request.Builder().url(httpBuilder.build()).build();
			try (Response response = this.config.client.newCall(request).execute()) {
				RestUtils.throwIfNotSuccessful(response);
				FeedResponse<DOMAINOBJECT> entity = objectMapper.readValue(
					response.body().string(),
					objectMapper
						.getTypeFactory()
						.constructParametricType(
							FeedResponse.class,
							this.config.domainobject
						)
				);
