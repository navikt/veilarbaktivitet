...
public class MoteSMSService implements HealthCheck {
	...
	protected void sendServicemeldinger(Date fra, Date til) {
		List<SmsAktivitetData> smsAktivitetData = moteSmsDAO.hentIkkeAvbrutteMoterMellom(
			fra,
			til
		);
	...
	private void trySendServicemelding(SmsAktivitetData aktivitetData) {
		try {
			...
			log.error(
				"feil med varsel paa motesms for aktivitetId " +
	...
	private void oppdaterMoteSmsMetrikker() {
		...
		if (smserSendt == null) {
			smserSendt =
				registry.gauge("antallSmsSendt", new AtomicLong(oppdatertSmsSendt));
		} else {
		...
		if (aktivterSendtSmsPaa == null) {
			aktivterSendtSmsPaa =
				registry.gauge(
					"antallAkteterSendtSmsPaa",
					new AtomicLong(oppdatertAktivterSendtSmsPaa)
				);
		} else {
	...
	public HealthCheckResult checkHealth() {
		return healty.get()
			...
			: HealthCheckResult.unhealthy("Sending av motesms feiler");
	}
}
