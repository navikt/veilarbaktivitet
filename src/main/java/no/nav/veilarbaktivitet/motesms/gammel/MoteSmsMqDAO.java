...
public class MoteSmsMqDAO {

    private final Database database;

    public MoteSmsMqDAO(Database database) {
        this.database = database;
    }

    List<SmsAktivitetData> hentIkkeAvbrutteMoterMellom(Date fra, Date til) {
        //language=sql
    ...
    public List<SmsAktivitetData> hentIkkeAvbrutteMoterMellomMedGamelNotifikasjon(Date fra, Date til) {
        //language=sql
        return database.query(
                "select " +
                        " AKTOR_ID " +
                        ...
                        " inner join GJELDENDE_MOTE_SMS on AKTIVITET.AKTIVITET_ID = GJELDENDE_MOTE_SMS.AKTIVITET_ID" +
                        " where AKTIVITET_TYPE_KODE  = 'MOTE'" +
                        " and GJELDENDE = 1" +
                        " and LIVSLOPSTATUS_KODE != 'AVBRUTT'" +
                        " and FRA_DATO between ? and ?" +
                        " and HISTORISK_DATO is null " +
                        "and BRUKERNOTIFIKASJON = 0" +
                        " order by FRA_DATO"
                ,
                this::mapper,
                fra,
                til
        ).stream().filter(SmsAktivitetData::skalSendeServicevarsel).toList();
    }
    ...
    public void slettGjeldende(Long aktivitetId) {
        MapSqlParameterSource params = new MapSqlParameterSource()
                .addValue("aktivitetId", aktivitetId);
        database.getNamedJdbcTemplate().update("""
delete from GJELDENDE_MOTE_SMS where AKTIVITET_ID = :aktivitetId
""", params);
    }
}
