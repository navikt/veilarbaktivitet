...
public class ArenaService {
	...
	public List<ArenaAktivitetDTO> hentAktiviteter(Person.Fnr fnr) {
		List<ArenaAktivitetDTO> aktiviteterFraArena = consumer.hentArenaAktiviteter(
			fnr
		);

		Person.AktorId aktorId = authService
			.getAktorIdForPersonBrukerService(fnr)
			.orElseThrow(
				() ->
		...
		List<ArenaForhaandsorienteringData> forhaandsorienteringData = dao.hentForhaandsorienteringer(
			aktorId
		);
	...
	public Optional<ArenaAktivitetDTO> hentAktivitet(
		...
		return hentAktiviteter(ident)
			.stream()
			.filter(
				arenaAktivitetDTO -> aktivitetId.equals(arenaAktivitetDTO.getId())
			)
			.findAny();
	}

	private Function<ArenaAktivitetDTO, ArenaAktivitetDTO> mergeMedForhaandsorientering(
		List<ArenaForhaandsorienteringData> forhaandsorienteringData
	) {
		return arenaAktivitetDTO ->
			arenaAktivitetDTO.setForhaandsorientering(
				forhaandsorienteringData
					.stream()
					.filter(
						arenaForhaandsorienteringData ->
							arenaForhaandsorienteringData
								.getArenaktivitetId()
								.equals(arenaAktivitetDTO.getId())
					)
					.findAny()
	...
	public ArenaAktivitetDTO lagreForhaandsorientering(
		...
		try {
			dao.insertForhaandsorientering(
				arenaaktivitetId,
				aktorId,
	...
	public ArenaAktivitetDTO markerSomLest(Person.Fnr fnr, String aktivitetId) {
		...
		if (!oppdatert) {
			log.warn(
				"kunne ikke markere forhondsorentering på arena aktivitet " +
				aktivitetId +
				" som lest"
			);
			throw new ResponseStatusException(
				HttpStatus.BAD_REQUEST,
				"Kan ikke markere aktiviteten som lest"
			);
		}
