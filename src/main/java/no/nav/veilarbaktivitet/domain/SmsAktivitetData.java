...
public class SmsAktivitetData {
	...
	public boolean skalSendeServicevarsel() {
		return (
			!moteTidAktivitet.equals(smsSendtMoteTid) ||
			!sendtMoteType(aktivitetKanal).equals(sendtMoteType(smsKanal))
		);
	}
	...
	public String formatertMoteTid() {
		return (
			DatoFormaterer.format(moteTidAktivitet) +
			" kl. " +
			KlokkeFormaterer.format(moteTidAktivitet)
		);
	}
	...
	private static final SimpleDateFormat DatoFormaterer = new SimpleDateFormat(
		"d. MMMM yyyy",
		norge
	);
	private static final SimpleDateFormat KlokkeFormaterer = new SimpleDateFormat(
		"HH:mm",
		norge
	);

	private static final String AKTIVITETSPLAN_URL = getRequiredProperty(
		"AKTIVITETSPLAN_URL"
	);
}
