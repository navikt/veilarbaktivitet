...
public class AktivitetDataMapper {
	...
	public static AktivitetData mapTilAktivitetData(AktivitetDTO aktivitetDTO) {
		val id = Optional
			.ofNullable(aktivitetDTO.getId())
			.filter(s -> !s.isEmpty())
			.map(Long::parseLong)
			.orElse(null);
		val versjon = Optional
			.ofNullable(aktivitetDTO.versjon)
			.map(Long::parseLong)
			.orElse(0L);
		...
		val aktivitetData = AktivitetData
			...
			.lagtInnAv(
				AuthContextHolderThreadLocal.instance().erEksternBruker()
					? InnsenderData.BRUKER
					: InnsenderData.NAV
			)
		...
		switch (aktivitetType) {
			case EGENAKTIVITET:
				...
				break;
			case JOBBSOEKING:
				aktivitetData.stillingsSoekAktivitetData(
					stillingsoekAktivitetData(aktivitetDTO)
				);
				break;
			case SOKEAVTALE:
				aktivitetData.sokeAvtaleAktivitetData(
					sokeAvtaleAktivitetData(aktivitetDTO)
				);
				break;
			case IJOBB:
				...
				break;
			case BEHANDLING:
				aktivitetData.behandlingAktivitetData(
					behandlingAktivitetData(aktivitetDTO)
				);
				break;
			case MOTE:
			case SAMTALEREFERAT:
	...
	private static EgenAktivitetData egenAktivitetData(
	...
	private static StillingsoekAktivitetData stillingsoekAktivitetData(
		AktivitetDTO aktivitetDTO
	) {
	...
	private static SokeAvtaleAktivitetData sokeAvtaleAktivitetData(
		AktivitetDTO aktivitetDTO
	) {
	...
	private static IJobbAktivitetData iJobbAktivitetData(
		AktivitetDTO aktivitetDTO
	) {
		return IJobbAktivitetData
			.builder()
			.jobbStatusType(
				Helpers.jobbStatusMap.getKey(aktivitetDTO.getJobbStatus())
			)
	...
	private static BehandlingAktivitetData behandlingAktivitetData(
		AktivitetDTO aktivitetDTO
	) {
