...
public class AktivitetDTOMapper {
	...
	public static AktivitetDTO mapTilAktivitetDTO(AktivitetData aktivitet) {
		...
		FunctionUtils
			.nullSafe(AktivitetDTOMapper::mapStillingSokData)
			.accept(aktivitetDTO, aktivitet.getStillingsSoekAktivitetData());
		FunctionUtils
			.nullSafe(AktivitetDTOMapper::mapEgenAktivitetData)
			.accept(aktivitetDTO, aktivitet.getEgenAktivitetData());
		FunctionUtils
			.nullSafe(AktivitetDTOMapper::mapSokeAvtaleData)
			.accept(aktivitetDTO, aktivitet.getSokeAvtaleAktivitetData());
		FunctionUtils
			.nullSafe(AktivitetDTOMapper::mapIJobbData)
			.accept(aktivitetDTO, aktivitet.getIJobbAktivitetData());
		FunctionUtils
			.nullSafe(AktivitetDTOMapper::mapBehandleAktivitetData)
			.accept(aktivitetDTO, aktivitet.getBehandlingAktivitetData());
		FunctionUtils
			.nullSafe(AktivitetDTOMapper::mapMoteData)
			.accept(aktivitetDTO, aktivitet.getMoteData());

		return aktivitetDTO;
	}

	private static void mapMoteData(
	...
	private static AktivitetDTO mapStillingSokData(
		AktivitetDTO aktivitetDTO,
		StillingsoekAktivitetData stillingsoekAktivitetData
	) {
		return aktivitetDTO
			.setEtikett(
				etikettMap.get(stillingsoekAktivitetData.getStillingsoekEtikett())
			)
