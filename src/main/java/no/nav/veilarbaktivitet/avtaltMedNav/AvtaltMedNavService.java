...
public class AvtaltMedNavService {
	...
	AktivitetDTO markerSomAvtaltMedNav(
		long aktivitetId,
		...
		return AktivitetDTOMapper.mapTilAktivitetDTO(
			dao.hentAktivitet(aktivitetId)
		);
	}

	AktivitetDTO markerSomLest(AktivitetData aktivitetData) {
		Forhaandsorientering fho = aktivitetData
			.getForhaandsorientering()
			.toBuilder()
			.lest(new Date())
			.build();
		...
		return AktivitetDTOMapper.mapTilAktivitetDTO(
			dao.hentAktivitet(aktivitetData.getId())
		);
	}
}
